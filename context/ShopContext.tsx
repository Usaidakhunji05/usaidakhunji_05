...
import { Shop, getShops, createShop, deleteShop, initDatabase } from "@/lib/db";
...
export function ShopProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadShops = useCallback(async () => {
    ...
    // Initialize SQLite only after the React tree has mounted. This prevents
    // a native database initialization failure from terminating the app
    // before Expo Router can render a useful screen.
    try {
      initDatabase();
      setShops(getShops());
    } catch (error) {
      console.warn("ShelfFlow database initialization failed:", error);
      setShops([]);
    }
  }, [isWeb]);
  ...
  const addShop = useCallback(async (shopName: string, shopType: string) => {
    if (isWeb) {
      ...
      try {
        ...
        console.warn("ShelfFlow could not create shop:", error);
      }
    }
  }, [isWeb]);
  ...
  const removeShop = useCallback(async (id: number) => {
    if (isWeb) {
      ...
      try {
        ...
        console.warn("ShelfFlow could not delete shop:", error);
      }
    }
