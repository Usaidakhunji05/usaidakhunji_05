...
export default function ProductFormScreen() {
  ...
  const pricePerBox = parseFloat(price || "0") * (parseInt(boxQuantity || "1") || 1);
  ...
  useEffect(() => {
    if (id) {
      ...
      if (p) {
        ...
        setUnit(p.unit);
        try {
          setMeta(p.metadata ? JSON.parse(p.metadata) : {});
        } catch {
          setMeta({});
        }
      }
  ...
  const handleSave = async () => {
    ...
    try {
      const product: Omit<Product, "id"> = {
        ...
        boxQuantity: parseInt(boxQuantity) || 1,
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        {/* ── Box Qty + Unit (conditional) ─────────────────────── */}
        {config.showBoxQty && (
          <View style={styles.rowFields}>
            <View style={[styles.fieldGroup, { flex: 1 }]}>
              <Text style={[styles.label, { color: colors.foreground }]}>{config.boxQtyLabel}</Text>
              <View style={[styles.inputRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <TextInput
                  style={[styles.input, { color: colors.foreground }]}
                  placeholder="1"
        ...
        {/* Price Per Box auto-calc card */}
        {config.showBoxQty && config.showUnit && parseFloat(price || "0") > 0 && (
          <View style={[styles.autoCalcCard, { backgroundColor: colors.primary + "12", borderColor: colors.primary + "30" }]}>
            <View style={styles.autoCalcRow}>
              <Text style={[styles.autoCalcLabel, { color: colors.mutedForeground }]}>Price Per {config.boxQtyLabel.split(" ")[0]}</Text>
              <Text style={[styles.autoCalcFormula, { color: colors.mutedForeground }]}>
                {boxQuantity || 1} × ₹{parseFloat(price || "0").toLocaleString()}
              </Text>
            </View>
            <Text style={[styles.autoCalcValue, { color: colors.primary }]}>
              ₹{pricePerBox.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </Text>
          </View>
        )}

        {/* ── Price (when not shown above) ─────────────────────── */}
        {(!config.showBoxQty || !config.showUnit) && (
          <View style={styles.fieldGroup}>
            <Text style={[styles.label, { color: colors.foreground }]}>Selling Price (₹)</Text>
            <View style={[styles.inputRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <TextInput
                style={[styles.input, { color: colors.foreground }]}
                placeholder="0.00"
        ...
        {config.showUnit && !config.showBoxQty && (
          <View style={styles.fieldGroup}>
        ...
        {/* Unit selector for box-qty types */}
        {config.showUnit && config.showBoxQty && (
          <View style={styles.fieldGroup}>
            <Text style={[styles.label, { color: colors.foreground }]}>Unit</Text>
            <View style={styles.chipRow}>
