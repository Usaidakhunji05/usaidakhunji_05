  ...
  const confirmSettle = async () => {
    ...
    try {
      if (settleTarget.kind === "sale") {
        settleCreditSale(settleTarget.id, settleDate);
      } else {
        settleCreditPurchase(settleTarget.id, settleDate);
      }
      ...
      <View style={styles.row}>
        ...
        <View style={styles.rowRight}>
          ...
          <TouchableOpacity
            ...
            onPress={() => openSettle(item.id, "sale", item.customerName || "customer", item.amount)}
            activeOpacity={0.8}
          >
  ...
  const renderPurchaseRow = (item: Purchase, index: number, total: number) => (
    <React.Fragment key={item.id}>
      ...
      <View style={styles.row}>
        ...
        <View style={styles.rowRight}>
          <Text style={[styles.rowAmount, { color: "#F59E0B" }]}>₹{(item.cost * item.qty).toLocaleString()}</Text>
          <TouchableOpacity
            style={[styles.settleBtn, { backgroundColor: "#F59E0B" }]}
            onPress={() => openSettle(item.id, "purchase", item.supplier || "supplier", item.cost * item.qty)}
            activeOpacity={0.8}
          >
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { backgroundColor: colors.card }]}>
            ...
            <View style={styles.modalField}>
              <Text style={[styles.modalLabel, { color: colors.foreground }]}>Settlement Date</Text>
              ...
              {settling ? (
                ...
                <>
                  ...
                  <Text style={styles.confirmBtnText}>
                    {settleTarget?.kind === "sale" ? "Mark as Collected" : "Mark as Paid"}
                  </Text>
                </>
              )}
            </TouchableOpacity>
